  ...
  outputs = { self, nixpkgs }:
    ...
    in {
      packages = forAllSystems (system:
        ...
        in {
          default = pkgs.stdenvNoCC.mkDerivation {
            ...
            version = "0.1.0";
            src = ./.;
            nativeBuildInputs = [ pkgs.jdk21 pkgs.kotlin pkgs.makeWrapper ];

            buildPhase = ''
              runHook preBuild
              mkdir -p build/classes
              for name in ${executableArgs}; do
                test -f "apps/$name/Main.kt" || { echo "missing apps/$name/Main.kt" >&2; exit 1; }
                mkdir -p "build/classes/$name"
                kotlinc apps/"$name"/*.kt -jvm-target 21 -include-runtime -d "build/classes/$name/app.jar"
              done
              runHook postBuild
            '';
            ...
            installPhase = ''
              ...
              for name in ${executableArgs}; do
                ...
                makeWrapper ${pkgs.jdk21}/bin/java "$out/bin/$name" \
                  --add-flags "-cp $out/lib/student-apps/$name/app.jar MainKt"
              done
              runHook postInstall
            '';
          };
      ...
      devShells = forAllSystems (system:
        let pkgs = import nixpkgs { inherit system; };
        in { default = pkgs.mkShell { packages = [ pkgs.jdk21 pkgs.kotlin ]; }; });
    };
}
